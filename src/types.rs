...
/// Data structure to hold the data resulting from a data query.
///
/// The data itself will be a tuple (or a `Vector` if all the elements are of the same type), but
/// it is necessary to provide the type parameter explicitely for the underlying parser to know how
/// to operate on the specific data received.
///
/// [Quandl API Reference](https://www.quandl.com/docs/api#data)
///
#[derive(Debug, Clone, PartialEq, RustcEncodable, RustcDecodable)]
pub struct Data<T: Decodable> {
    /// The data requested.
    ///
    pub data: Vec<T>,

    /// A `Vector` containing the name of every column of the data.
    ///
    pub column_names: Vec<String>,

    /// The start date of the data as a `String` of the form `format!("{:#4}-{:#2}-{:#2}", year,
    /// month, day);`.
    ///
    pub start_date: String,

    /// The end date of the data as a `String` of the form `format!("{:#4}-{:#2}-{:#2}", year,
    /// month, day);`.
    ///
    pub end_date: String,

    /// The interval between two consecutive data point in the original dataset.
    ///
    pub frequency: Frequency,

    /// The ascending or descending ordering of the data (with respect to the date for time
    /// series).
    ///
    pub order: Order,

    /// The number of rows to return. Is identical to the `limit` field.
    ///
    pub rows: Option<usize>,

    /// The number of rows to return. Is identical to the `rows` field.
    ///
    pub limit: Option<usize>,

    /// The interval between two consecutive data point in the transformed (returned) dataset.
    ///
    pub collapse: Option<Frequency>,

    /// The transform performed on this dataset; see also the `Transform` enum.
    ///
    pub transform: Option<Transform>,

    /// The column index that was returned. Note that the 'date' column (index 0) is always
    /// returned.
    ///
    pub column_index: Option<usize>,
}

/// Data structure to hold the data and metadata resulting from a data and metadata query.
///
/// The data itself will be a tuple (or a `Vector` if all the elements are of the same type), but
/// it is necessary to provide the type parameter explicitely for the underlying parser to know how
/// to operate on the specific data received.
...
///
#[derive(Debug, Clone, PartialEq, RustcEncodable, RustcDecodable)]
pub struct DataAndMetadata<T: Decodable> {
    /// The data requested.
    ///
    pub data: Vec<T>,
